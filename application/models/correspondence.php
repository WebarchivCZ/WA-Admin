...
class Correspondence_Model extends Table_Model {
	...
	protected $belongs_to = array('correspondence_type',
		'resource');
	...
	public $headers = array('resource', 'date',
		'correspondence_type', 'result');
	...
	public function search($pattern, & $count, $limit = 20,
						   $offset = 0)
	{
		// TODO - prazvlastni vec. nefunguje count_all
		$count = ORM::factory('resource')
			->like('title', $pattern)
			->join('correspondence', 'correspondence.resource_id = resources.id')
			->groupby('resources.id')->find_all()->count();
		$records = ORM::factory('resource')
			->like('title', $pattern)
			->join('correspondence', 'correspondence.resource_id = resources.id')
