...
class Contract_Model extends Table_Model {
    ...
    public static function get_contract($year, $contract_no) {
        if (is_numeric($year) and is_numeric($contract_no)) {
            $contract = ORM::factory('contract')->where(array('contract_no' => $contract_no, 'year' => $year))->find();
            if ($contract->loaded) {
                return $contract;
            } else {
                return FALSE;
    ...
    public static function domain_get_blanco($url) {
        ...
        $result = Database::instance()->query($sql)->current();
        return ORM::factory('contract')->where('id', $result->id)->find_all();
    }
    ...
    public function search($pattern, & $count, $limit = 20, $offset = 0) {
        ...
        $count = $this->where($conditions)->select('DISTINCT contracts.*')->orlike(array('resources.title' => $pattern, 'publishers.name' => $pattern))->join('resources', 'resources.contract_id = contracts.id')->join('publishers', 'resources.publisher_id = publishers.id')->find_all()->count();

        $records = $this->where($conditions)->select('DISTINCT contracts.*')->orlike(array('resources.title' => $pattern, 'publishers.name' => $pattern))->join('resources', 'resources.contract_id = contracts.id')->join('publishers', 'resources.publisher_id = publishers.id')->find_all($limit, $offset);
