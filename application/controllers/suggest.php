...
class Suggest_Controller extends Template_Controller {
	...
	{
		$resources = ORM::factory('resource')->join('publishers', 'resources.publisher_id = publishers.id')->orlike(array('url'             => $url,
																														  'title'           => $title,
																														  'publishers.name' => $publisher_name))->find_all();
	...
	public function get_subcategories($conspectus_id = NULL)
	{
		...
		{
			$sub_array[] = array("optionValue"   => $subcategory->id,
								 "optionDisplay" => $subcategory->title);
		}
