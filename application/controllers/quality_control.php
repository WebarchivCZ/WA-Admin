...
class Quality_Control_Controller extends Template_Controller {
	...
	public function add($resource_id = NULL)
	{
		...
		$form = $this->generate_add_form($resource);

		if ($form->validate())
		{
			$is_saved = $this->save_new($form, $resource_id);
			if ($is_saved)
			{
	...
	public function edit($qa_check_id)
	{
		...
		$form = $this->generate_edit_form($resource, $qa_check);

		if ($form->validate())
		{
			$is_saved = $this->save_edit($form, $qa_check);
			if ($is_saved)
			{
				message::set_flash('Kontrola byla úspěšně uložena.');
			} else
